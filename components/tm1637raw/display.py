...
TM1637RawDisplayRef = TM1637RawDisplay.operator("ref")

CONFIG_SCHEMA = display.BASIC_DISPLAY_SCHEMA.extend(
    {
        cv.GenerateID(): cv.declare_id(TM1637RawDisplay),
...
async def to_code(config):
    ...
    if CONF_LAMBDA in config:
        lambda_ = await cg.process_lambda(
            config[CONF_LAMBDA], [(TM1637RawDisplayRef, "it")], return_type=cg.void
        )
