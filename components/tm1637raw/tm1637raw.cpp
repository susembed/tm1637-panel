...
namespace esphome
{
  namespace tm1637raw
  {
    ...
    static const char *const TAG = "display.tm1637raw";
    const uint8_t TM1637RAW_CMD_DATA = 0x40; //!< Display data command
    const uint8_t TM1637RAW_CMD_CTRL = 0x80; //!< Display control command
    const uint8_t TM1637RAW_CMD_ADDR = 0xc0; //!< Display address command
    const uint8_t TM1637RAW_UNKNOWN_CHAR = 0b11111111;
    ...
    //      D   X
    // XABCDEFG
    const uint8_t TM1637RAW_ASCII_TO_RAW[] PROGMEM = {
        0b00000000,             // ' ', ord 0x20
        0b10110000,             // '!', ord 0x21
        0b00100010,             // '"', ord 0x22
        TM1637RAW_UNKNOWN_CHAR, // '#', ord 0x23
        TM1637RAW_UNKNOWN_CHAR, // '$', ord 0x24
        0b01001001,             // '%', ord 0x25
        TM1637RAW_UNKNOWN_CHAR, // '&', ord 0x26
        0b00000010,             // ''', ord 0x27
        0b01001110,             // '(', ord 0x28
        0b01111000,             // ')', ord 0x29
        0b01000000,             // '*', ord 0x2A
        TM1637RAW_UNKNOWN_CHAR, // '+', ord 0x2B
        0b00010000,             // ',', ord 0x2C
        0b00000001,             // '-', ord 0x2D
        0b10000000,             // '.', ord 0x2E
        TM1637RAW_UNKNOWN_CHAR, // '/', ord 0x2F
        ...
        0b01001000,             // ':', ord 0x3A
        0b01011000,             // ';', ord 0x3B
        TM1637RAW_UNKNOWN_CHAR, // '<', ord 0x3C
        TM1637RAW_UNKNOWN_CHAR, // '=', ord 0x3D
        TM1637RAW_UNKNOWN_CHAR, // '>', ord 0x3E
        0b01100101,             // '?', ord 0x3F
        0b01101111,             // '@', ord 0x40
        ...
        TM1637RAW_UNKNOWN_CHAR, // 'K', ord 0x4B
        0b00001110,             // 'L', ord 0x4C
        TM1637RAW_UNKNOWN_CHAR, // 'M', ord 0x4D
        ...
        TM1637RAW_UNKNOWN_CHAR, // 'X', ord 0x58
        0b00100111,             // 'Y', ord 0x59
        0b01101101,             // 'Z', ord 0x5A
        0b01001110,             // '[', ord 0x5B
        TM1637RAW_UNKNOWN_CHAR, // '\\', ord 0x5C
        0b01111000,             // ']', ord 0x5D
        TM1637RAW_UNKNOWN_CHAR, // '^', ord 0x5E
        ...
        TM1637RAW_UNKNOWN_CHAR, // 'k', ord 0x6B
        0b00001110,             // 'l', ord 0x6C
        TM1637RAW_UNKNOWN_CHAR, // 'm', ord 0x6D
        0b00010101,             // 'n', ord 0x6E
        0b00011101,             // 'o', ord 0x6F
        0b01100111,             // 'p', ord 0x70
        TM1637RAW_UNKNOWN_CHAR, // 'q', ord 0x71
        0b00000101,             // 'r', ord 0x72
        0b01011011,             // 's', ord 0x73
        0b00000111,             // 't', ord 0x74
        0b00011100,             // 'u', ord 0x75
        0b00011100,             // 'v', ord 0x76
        TM1637RAW_UNKNOWN_CHAR, // 'w', ord 0x77
        TM1637RAW_UNKNOWN_CHAR, // 'x', ord 0x78
        0b00100111,             // 'y', ord 0x79
        TM1637RAW_UNKNOWN_CHAR, // 'z', ord 0x7A
        0b00110001,             // '{', ord 0x7B
        0b00000110,             // '|', ord 0x7C
        0b00000111,             // '}', ord 0x7D
        0b01100011,             // '~', ord 0x7E (degree symbol)
    };
    void TM1637RawDisplay::setup()
    {
    ...
    void TM1637RawDisplay::dump_config()
    {
      ESP_LOGCONFIG(TAG,
                    "TM1637RAW:\n"
                    "  Intensity: %d\n"
    ...
    uint8_t TM1637RawDisplay::get_keys()
    {
      this->start_();
      this->send_byte_(TM1637RAW_CMD_DATA | TM1637_DATA_READ_KEYS);
      this->start_();
    ...
    float TM1637RawDisplay::get_setup_priority() const { return setup_priority::PROCESSOR; }
    void TM1637RawDisplay::bit_delay_() { delayMicroseconds(100); }
    void TM1637RawDisplay::start_()
    {
      this->dio_pin_->pin_mode(gpio::FLAG_OUTPUT);
      this->bit_delay_();
    }

    void TM1637RawDisplay::stop_()
    {
      this->dio_pin_->pin_mode(gpio::FLAG_OUTPUT);
      bit_delay_();
    ...
    void TM1637RawDisplay::display()
    {
      ...
      this->start_();
      this->send_byte_(TM1637RAW_CMD_CTRL + ((this->intensity_ & 0x7) | (this->on_ ? 0x08 : 0x00)));
      this->stop_();
    }
    bool TM1637RawDisplay::send_byte_(uint8_t b)
    {
      uint8_t data = b;
    ...
    uint8_t TM1637RawDisplay::read_byte_()
    {
      uint8_t retval = 0;
    ...
    uint8_t TM1637RawDisplay::print(uint8_t start_pos, const char *str)
    {
      ...
      for (; *str != '\0'; str++)
      {
        uint8_t data = TM1637RAW_UNKNOWN_CHAR;
        if (*str >= ' ' && *str <= '~')
          data = progmem_read_byte(&TM1637RAW_ASCII_TO_RAW[*str - ' ']);

        if (data == TM1637RAW_UNKNOWN_CHAR)
        {
    ...
    uint8_t TM1637RawDisplay::print(const char *str) { return this->print(0, str); }
    uint8_t TM1637RawDisplay::printf(uint8_t pos, const char *format, ...)
    {
      va_list arg;
      va_start(arg, format);
      char buffer[64];
    ...
    uint8_t TM1637RawDisplay::printf(const char *format, ...)
    {
      va_list arg;
      va_start(arg, format);
      char buffer[64];
    ...
    uint8_t TM1637RawDisplay::strftime(uint8_t pos, const char *format, ESPTime time)
    {
      char buffer[64];
    ...
    uint8_t TM1637RawDisplay::strftime(const char *format, ESPTime time) { return this->strftime(0, format, time); }

    // -- raw segments control ---------------------------------------------------
    // Allow direct control of a character's raw segment byte from lambdas, e.g.
    // it.raw(0b00000010, 1);
    // `segments` is the TM1637 segment byte in the same bit layout used in
    // `buffer_` (XGFEDCBA). `pos` is 0-indexed from the leftmost character.
    void TM1637RawDisplay::raw(uint8_t segments, uint8_t pos)
    {
      if (pos >= 6 || pos >= this->length_)
      {
        ESP_LOGE(TAG, "raw(): position %d out of bounds (length %d)", pos, this->length_);
        return;
      }
      this->buffer_[pos] = segments;
    }
